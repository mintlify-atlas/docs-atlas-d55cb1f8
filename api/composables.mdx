...
Control the AI assistant panel state and functionality.

**Location:** `layer/modules/assistant/runtime/composables/useAssistant.ts`
...
<ResponseField name="isOpen" type="Ref<boolean>">
  Whether the assistant panel is currently open.
</ResponseField>

<ResponseField name="isExpanded" type="Ref<boolean>">
  Whether the assistant panel is in expanded mode (520px vs 360px).
</ResponseField>

<ResponseField name="isMobile" type="Ref<boolean>">
  Whether the current viewport is mobile (max-width: 767px).
</ResponseField>

<ResponseField name="panelWidth" type="Ref<number>">
  Current panel width in pixels (360 for compact, 520 for expanded).
</ResponseField>

<ResponseField name="shouldPushContent" type="Ref<boolean>">
  Whether the assistant panel should push content aside (true when open on desktop).
</ResponseField>

<ResponseField name="messages" type="Ref<UIMessage[]>">
  Array of chat messages in the current conversation.
</ResponseField>

<ResponseField name="pendingMessage" type="Ref<string | undefined>">
  Message waiting to be sent when the panel opens.
</ResponseField>

<ResponseField name="faqQuestions" type="Ref<FaqCategory[]>">
  FAQ questions configured in `app.config.ts`, normalized to category format.
  
  Automatically handles localized FAQ questions based on current locale.
</ResponseField>

### Methods

<ResponseField name="open" type="(initialMessage?: string, clearPrevious?: boolean) => void">
...
<ResponseField name="toggleExpanded" type="() => void">
...
<ResponseField name="clearPending" type="() => void">
